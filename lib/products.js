...
// Precio del formato "frasco completo" (null si el producto no tiene uno
// disponible). Se usa en el catálogo para mostrar ese precio aparte del
// "desde" cuando el filtro de formato está activo.
export function precioFrascoCompleto(producto) {
  const disponibles = producto.formatos.filter(
    (f) => f.tipo === "frasco completo" && f.stock !== 0 && f.precio != null
  );
  if (disponibles.length === 0) return null;
  return Math.min(...disponibles.map((f) => f.precio));
}
